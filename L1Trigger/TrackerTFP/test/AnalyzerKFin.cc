...
namespace trackerTFP {
  ...
  void AnalyzerKFin::beginRun(const Run& iEvent, const EventSetup& iSetup) {
    ...
    const int numChannels = dataFormats_->numChannel(Process::kf) * setup_->numLayers();
  ...
  void AnalyzerKFin::analyze(const Event& iEvent, const EventSetup& iSetup) {
    ...
    for (int region = 0; region < setup_->numRegions(); region++) {
      ...
      for (int channel = 0; channel < dataFormats_->numChannel(Process::kf); channel++) {
        const int offset = (region * dataFormats_->numChannel(Process::kf) + channel) * setup_->numLayers();
