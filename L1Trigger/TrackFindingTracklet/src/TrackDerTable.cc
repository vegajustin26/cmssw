...
void TrackDerTable::getVarianceMatrix(
...
void TrackDerTable::calculateDerivativesMS(const Settings* settings,
  ...
  for (unsigned int i = 0; i < ndisks; i++) {
    for (unsigned int j = 0; j < 5; j++) {
      if (std::abs(std::abs(z[i]) - settings->zmean(j)) < settings->dzmax()) {
        disk[j] = true;
        edm::LogVerbatim("Tracklet") << "z zmean ndisks" << z[i] << " " << settings->zmean(j) << " " << ndisks << " "
                                     << nlayers;
      }
    }
  }
  ...
  edm::LogVerbatim("Tracklet") << "layer : " << nlayers << " " << layer[0] << layer[1] << layer[2] << layer[3]
                               << layer[4] << layer[5];
