...
bool MatchProcessor::matchCalculator(Tracklet* tracklet,Stub* fpgastub, L1TStub* stub) {
	...
	r=fpgastub->rapprox();
      ...
      double dphiapprox=Trklet::phiRange(phi-(tracklet->phiprojapprox(layer_)+
	...
	assert(std::abs(dz)<settings_->dzmax());
      ...
      double dphiapprox=Trklet::phiRange(phi-(tracklet->phiprojapproxdisk(disk)+dz*tracklet->phiprojderapproxdisk(disk)));
...
double MatchProcessor::bend(double r, double rinv) {
