...
void TrackletEventProcessor::event(SLHCEvent& ev) {
  ...
  */  
  ...
  if (settings_->writeMonitorData("StubsLayer")) {
    ...
    out <<stublayer[0]<<" "<<stublayer[1]<<" "<<stublayer[2]<<" "<<stublayer[3]<<" "<<stublayer[4]<<" "<<stublayer[5]<<endl;
  }     
  
  if (settings_->writeMonitorData("StubsLayerSector")) {
    static ofstream out("stubslayerpersector.txt");
    for(unsigned int jj=0;jj<settings_->NSector();jj++){
      out <<stublayer1[0][jj]<<" "<<stublayer1[1][jj]<<" "<<stublayer1[2][jj]<<" "
	  <<stublayer1[3][jj]<<" "<<stublayer1[4][jj]<<" "<<stublayer1[5][jj]<<endl; 
    }
    static ofstream out1("stubsdiskpersector.txt");
    for(unsigned int jj=0;jj<settings_->NSector();jj++){
      out1 <<stubdisk1[0][jj]<<" "<<stubdisk1[1][jj]<<" "<<stubdisk1[2][jj]<<" "<<stubdisk1[3][jj]<<" "<<stubdisk1[4][jj]<<endl; 
    }
  }     
