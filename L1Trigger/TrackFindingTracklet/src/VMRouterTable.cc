...
void VMRouterTable::init(unsigned int layerdisk) {
  ...
  if (layerdisk<6) {
    ...
    zmin_ = settings_->zmean(layerdisk - 6) - settings_->dzmax();
    zmax_ = settings_->zmean(layerdisk - 6) + settings_->dzmax();
  }
  ...
  for (int izbin = 0; izbin < zbins_; izbin++) {
	...
	double rproj = r * settings_->zmean(layerdisk - 6) / z;
...
int VMRouterTable::getLookup(unsigned int layerdisk, double z, double r, int iseed){
  ...
  if (layerdisk<6) {
    ...
    if (iseed==1 && std::abs(z)<52.0) return -1;
    ...
    if (zbin1>=NBINS) return -1;
    if (zbin2<0) return -1;

    if (zbin2>=NBINS) zbin2=NBINS-1;
    if (zbin1<0) zbin1=0;
    ...
    if (std::abs(z)<2.0*z0cut) return -1;

    double zmean=settings_->zmean(layerdisk-6);
    if (z<0.0) zmean=-zmean;
    ...
    if (iseed==6) rmaxdisk=settings_->rmaxdiskl1overlapvm();
    if (iseed==7) rmindisk=settings_->rmindiskl2overlapvm();
    if (iseed==10) rmaxdisk=settings_->rmaxdisk();
    
    if (rmin>rmaxdisk) return -1;
    if (rmax>rmaxdisk) rmax=rmaxdisk;

    if (rmax<rmindisk) return -1;
    if (rmin<rmindisk) rmin=rmindisk;    
