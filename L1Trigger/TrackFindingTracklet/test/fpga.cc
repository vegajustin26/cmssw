...
#include "../plugins/WriteDesign.icc"
  ...
  using namespace std;
  if (argc < 4)
    cout << "Need to specify the input ascii file and the number of events to run on and if you want to filter on MC "
            "truth"
         << endl;
...
// -----------------------------------------------------------------
    ...
    if (selectmu == 1) {
      ...
      if (debug1) {
        cout << "nstub simtrkid pt phi eta t vz:" << ev.nstubs() << " " << simtrk.trackid() << " " << simtrk.pt() << " "
             << simtrk.phi() << " " << simtrk.eta() << " " << sinh(simtrk.eta()) << " " << simtrk.vz() << endl;
      }
    ...
    cout << "Process event: " << eventnum << " with " << ev.nstubs() << " stubs and " << ev.nsimtracks() << " simtracks"
         << endl;
...
#include "FPGATree.icc"
    ...
    if (writeResEff) {
      outeff << simtrk.pt() * simtrk.trackid() / fabs(simtrk.trackid()) << " " << simtrk.eta() << " " << simtrk.phi();
      if (match)
        outeff << " 1" << endl;
      else
        outeff << " 0" << endl;
    ...
    for (unsigned int l = 0; l < tracks.size(); l++) {
      ...
      if (writePars) {
        ...
        while (phisec < 0.0)
          phisec += two_pi / NSector;
        outpars << tracks[l]->duplicate() << " " << asinh(tracks[l]->it() * ktpars) << " " << phi << " "
                << tracks[l]->iz0() * kz << " " << phisec / (two_pi / NSector) << " " << tracks[l]->irinv() * krinvpars
                << endl;
      }
    ...
    cout << "Number layers/disks hit = " << nlayershit << " number of found tracks : " << tracks.size() << " unique "
         << ntrack << endl;
  ...
  cout << "Reading               " << setw(10) << readTimer.ntimes() << setw(20) << setprecision(3)
       << readTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << readTimer.tottime() << endl;
  cout << "Cleaning              " << setw(10) << cleanTimer.ntimes() << setw(20) << setprecision(3)
       << cleanTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << cleanTimer.tottime() << endl;
  cout << "Add Stubs             " << setw(10) << addStubTimer.ntimes() << setw(20) << setprecision(3)
       << addStubTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << addStubTimer.tottime() << endl;
  cout << "VMRouter              " << setw(10) << VMRouterTimer.ntimes() << setw(20) << setprecision(3)
       << VMRouterTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << VMRouterTimer.tottime() << endl;
  cout << "TrackletEngine        " << setw(10) << TETimer.ntimes() << setw(20) << setprecision(3)
       << TETimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << TETimer.tottime() << endl;
  cout << "TrackletEngineDisplaced" << setw(10) << TEDTimer.ntimes() << setw(20) << setprecision(3)
       << TEDTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << TEDTimer.tottime() << endl;
  cout << "TripletEngine         " << setw(10) << TRETimer.ntimes() << setw(20) << setprecision(3)
       << TRETimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << TRETimer.tottime() << endl;
  cout << "TrackletCalculator    " << setw(10) << TCTimer.ntimes() << setw(20) << setprecision(3)
       << TCTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << TCTimer.tottime() << endl;
  cout << "TrackletCalculatorDisplaced" << setw(10) << TCDTimer.ntimes() << setw(20) << setprecision(3)
       << TCDTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << TCDTimer.tottime() << endl;
  cout << "ProjectionRouter      " << setw(10) << PRTimer.ntimes() << setw(20) << setprecision(3)
       << PRTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << PRTimer.tottime() << endl;
  cout << "MatchEngine           " << setw(10) << METimer.ntimes() << setw(20) << setprecision(3)
       << METimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << METimer.tottime() << endl;
  cout << "MatchCalculator       " << setw(10) << MCTimer.ntimes() << setw(20) << setprecision(3)
       << MCTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << MCTimer.tottime() << endl;
  cout << "MatchProcessor        " << setw(10) << MPTimer.ntimes() << setw(20) << setprecision(3)
       << MPTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << MPTimer.tottime() << endl;
  cout << "FitTrack              " << setw(10) << FTTimer.ntimes() << setw(20) << setprecision(3)
       << FTTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << FTTimer.tottime() << endl;
  cout << "PurgeDuplicate        " << setw(10) << PDTimer.ntimes() << setw(20) << setprecision(3)
       << PDTimer.avgtime() * 1000.0 << setw(20) << setprecision(3) << PDTimer.tottime() << endl;
