...
public:
  VMStubsTEMemory(string name, unsigned int iSector, double phimin, double phimax) : MemoryBase(name, iSector) {
    phimin_ = phimin;
    phimax_ = phimax;
  ...
  bool addVMStub(VMStubTE vmstub, int bin) {
    ...
    if (!pass) {
      if (debug1)
        cout << getName()
             << " Stub failed bend cut. bend = " << Stub::benddecode(vmstub.bend().value(), vmstub.isPSmodule())
             << endl;
      return false;
    }
    ...
    if (overlap_) {
      if (disk_ == 1) {
        ...
        if (debug1)
          cout << getName() << " Stub in disk = " << disk_ << "  in bin = " << bin << endl;
      } else if (layer_ == 2) {
        stubsbinnedvm_[bin].push_back(vmstub);
      }
    } else {
    ...
    if (debug1)
      cout << "Adding stubs to " << getName() << endl;
    ...
    if (!extended_) {
      if (overlap_) {
        if (disk_ == 1) {
          ...
          stubsbinnedvm_[bin].push_back(vmstub);
          if (debug1)
            cout << getName() << " Stub with lookup = " << binlookup.value() << " in disk = " << disk_
                 << "  in bin = " << bin << endl;
        }
      } else {
        if (vmstub.stub().first->isBarrel()) {
