...
public:
  ProjectionRouter(string name, unsigned int iSector) : ProcessBase(name, iSector) {
  ...
  void addInput(MemoryBase* memory, string input) {
    ...
    if (input == "projin" || input == "proj1in" || input == "proj2in" || input == "proj3in" || input == "proj4in" ||
        input == "proj5in" || input == "proj6in" || input == "proj7in" || input == "proj8in" || input == "proj9in" ||
        input == "proj10in" || input == "proj11in" || input == "proj12in" || input == "proj13in" ||
        input == "proj14in" || input == "proj15in" || input == "proj16in" || input == "proj17in" ||
        input == "proj18in" || input == "proj19in" || input == "proj20in" || input == "proj21in" ||
        input == "proj22in" || input == "proj23in" || input == "proj24in" || input == "proj25in" ||
        input == "proj26in" || input == "proj27in" || input == "proj28in" || input == "proj29in" ||
        input == "proj30in" || input == "proj31in" || input == "proj32in" || input == "proj33in" ||
        input == "proj34in" || input == "proj35in" || input == "proj36in" || input == "proj37in" ||
        input == "proj38in" || input == "proj39in" || input == "proj40in" || input == "proj41in" ||
        input == "proj42in" || input == "proj43in" || input == "proj44in" || input == "proj45in" ||
        input == "proj46in" || input == "proj47in" || input == "proj48in") {
    ...
    if (layer_ != 0) {
      ...
      for (unsigned int j = 0; j < inputproj_.size(); j++) {
        for (unsigned int i = 0; i < inputproj_[j]->nTracklets(); i++) {
          ...
          int disk = disk_;
          if (inputproj_[j]->getFPGATracklet(i)->t() < 0.0)
            disk = -disk_;
          ...
          if (writeSeeds) {
            ...
            fout << __FILE__ << ":" << __LINE__ << " " << name_ << "_" << iSector_ << " "
                 << inputproj_[j]->getFPGATracklet(i)->getISeed() << endl;
            fout.close();
          }
          allproj_->addTracklet(inputproj_[j]->getFPGATracklet(i));
          ...
          int rindex = (tracklet->fpgarprojdisk(disk_).value() >> (tracklet->fpgarprojdisk(disk_).nbits() - nrbits_)) &
                       ((1 << nrbits_) - 1);

          int phiderindex = (tracklet->fpgaphiprojderdisk(disk_).value() >>
                             (tracklet->fpgaphiprojderdisk(disk_).nbits() - nphiderbits_)) &
                            ((1 << nphiderbits_) - 1);
