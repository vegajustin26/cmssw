...
public:
  InputLinkMemory(string name, unsigned int iSector, double phimin, double phimax) : MemoryBase(name, iSector) {
    phimin_ = phimin;
    phimax_ = phimax;
  ...
  bool addStub(L1TStub& al1stub, Stub& stub, string dtc = "") {
    ...
    if (stub.layer().value() == -1 && isLayer())
      return false;
    if (stub.layer().value() != -1 && isDisk())
      return false;
    if (stub.layer().value() != -1) {
      if (stub.layer().value() + 1 != layerdisk())
        return false;
    } else {
      if (abs(stub.disk().value()) != layerdisk())
        return false;
    }
    ...
    if (phibin != phiregion() && phibin != phiregionoverlap())
      return false;
    ...
    if (debug1) {
      cout << "Will add stub in " << getName() << " phimin_ phimax_ " << phimin_ << " " << phimax_ << " "
           << "iphiwmRaw = " << iphivmRaw << " phi=" << al1stub.phi() << " z=" << al1stub.z() << " r=" << al1stub.r()
           << endl;
    }
  ...
  void writeInputStubs(bool first, bool w2, bool padded) {
    ...
    if (!w2) {
      ...
      //Stub counts
      xline = (xline + 1) & 15;
      xword = (nlay2[5]) | (nlay2[4] << 6) | (nlay2[3] << 12) | (nlay2[2] << 18) | (nlay2[1] << 24) | (nlay2[0] << 30);
      out_ << std::hex << xline << "000";
      out_.fill('0');
      out_.width(4);
      out_ << (xword >> 20);
