...
namespace trklet {
  ...
  public:
    InputLinkMemory(const std::string& name, const Settings* const settings, unsigned int iSector, double, double);

    ~InputLinkMemory() override = default;

    bool addStub(const Settings* settings, Globals* globals, L1TStub& al1stub, Stub& stub, const std::string& dtc);
