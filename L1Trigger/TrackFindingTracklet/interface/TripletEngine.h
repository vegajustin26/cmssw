...
public:
  TripletEngine(string name, unsigned int iSector) : ProcessBase(name, iSector) {
  ...
  void execute() {
    ...
    int hacksum = 0;
    if (print) {
      cout << "In TripletEngine::execute : " << getName() << " " << nThirdStubs << ":\n";
                ...
                if (!table_[index]) {
                  if (debug1) {
                    cout << "Stub pair rejected because of stub pt cut bends : "
                         ...
                         << Stub::benddecode(thirdvmstub.bend().value(), thirdvmstub.isPSmodule()) << endl;
                  }
                  if (!writeTripletTables)
                    continue;
                }
