...
public:
  ...
  bool addStub(L1TStub stub, string dtc) {
    ...
    if (((phi > phimin_ - dphi) && (phi < phimax_ + dphi)) ||
      ...
      for (unsigned int i = 0; i < tmp.size(); i++) {
        ...
        if (IL_[tmp[i]]->addStub(stub, fpgastub, dtc))
          add = true;
      }
    }
  ...
  void addProc(string procType, string procName) {
    ...
    } else if (procType == "TrackletCalculator:" || procType == "TrackletDiskCalculator:") {
    ...
    } else if (procType == "MatchCalculator:" || procType == "DiskMatchCalculator:") {
