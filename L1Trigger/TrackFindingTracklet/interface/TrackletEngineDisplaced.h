...
public:
  TrackletEngineDisplaced(string name, unsigned int iSector) : ProcessBase(name, iSector) {
    ...
    if ((layer1_ == 3 && layer2_ == 4) || (layer1_ == 5 && layer2_ == 6) || (layer1_ == 2 && layer2_ == 3)) {
  ...
  ~TrackletEngineDisplaced() { table_.clear(); }
  ...
  void execute() {
    if (!((doL2L3D1 && (layer1_ == 2) && (layer2_ == 3)) || (doL3L4L2 && (layer1_ == 3) && (layer2_ == 4)) ||
          (doL5L6L4 && (layer1_ == 5) && (layer2_ == 6)) || (doD1D2L2 && (disk1_ == 1) && (disk2_ == 2)))) {
      return;
    }
              ...
              if (table_.at(index).empty()) {
                if (debug1) {
                  cout << "Stub pair rejected because of stub pt cut bends : "
                       ...
                       << Stub::benddecode(secondvmstub.bend().value(), secondvmstub.isPSmodule()) << endl;
                }
          ...
          for (int ibin = start; ibin <= last; ibin++) {
            if (debug1)
              cout << getName() << " looking for matching stub in bin " << ibin << " with "
                   << secondvmstubs_->nVMStubsBinned(ibin) << " stubs" << endl;
            for (unsigned int j = 0; j < secondvmstubs_->nVMStubsBinned(ibin); j++) {
              if (countall >= MAXTE)
                break;
              countall++;
    ...
    if (countall > 5000) {
      cout << "In TrackletEngineDisplaced::execute : " << getName() << " " << nInnerStubs << " "
           << secondvmstubs_->nVMStubs() << " " << countall << " " << countpass << endl;
      for (unsigned int iInnerMem = 0; iInnerMem < firstvmstubs_.size(); iInnerMem++) {
        for (unsigned int i = 0; i < firstvmstubs_.at(iInnerMem)->nVMStubs(); i++) {
          VMStubTE firstvmstub = firstvmstubs_.at(iInnerMem)->getVMStubTE(i);
          cout << "In TrackletEngineDisplaced::execute first stub : " << firstvmstub.stub().second->r() << " "
      ...
      for (unsigned int i = 0; i < secondvmstubs_->nVMStubs(); i++) {
        ...
        cout << "In TrackletEngineDisplaced::execute second stub : " << secondvmstub.stub().second->r() << " "
